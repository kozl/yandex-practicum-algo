...
/*
Успешная посылка: https://contest.yandex.ru/contest/24810/run-report/105920005/

Для решения реализовал кучу heap, в которую по одному сначала помещаю элементы, затем
по одному удаляю и таким образом в полученном массиве получаю элементы отсортированные по убыванию

В функции less реализовал операцию сравнения участников соревнования, maxChild возвращает максимального
потомка из имеющихся, siftUp и siftDown реализуют операции просеивания вверх и вниз при добавлении и удалении
элемента соотвественно.

Вычислительная сложность: O(nlog(n))
